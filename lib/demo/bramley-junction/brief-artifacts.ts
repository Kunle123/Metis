...
export function buildBramleyBriefArtifact(params: {
  lede: string;
  /** Short one-paragraph executive summary (not the long preview body). */
  executiveSummary: string;
  lastRevisionLabel: string;
  openGapsLabel: string;
  /** Status line content is demo-authored; keep deterministic. */
  ...
  const decisions = params.recommendedActionsBullets.length
    ...
    : "1) Maintain a controlled line; update as sources change.";
