...
export function IssueRecordHome({
  ...
  const canAddMaterial = canWrite && !isArchived;
  ...
  return (
    <SurfaceCard className="overflow-hidden">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {isArchived ? (
                <Badge className="border border-[--metis-status-warning-border] bg-[color-mix(in_oklab,var(--metis-status-warning-bg)_48%,transparent)] text-[--metis-status-warning-fg]">
                  Archived
                </Badge>
              ) : null}
          ...
          {canAddMaterial ? (
            ...
            <p className="max-w-[14rem] text-[0.72rem] leading-snug text-[--metis-text-tertiary] sm:text-right">
              Reopen this issue to add new material.
            </p>
          ) : !canWrite ? (
            <p className="max-w-[14rem] text-[0.72rem] leading-snug text-[--metis-text-tertiary] sm:text-right">
      ...
      <div className="space-y-6 px-6 py-6 sm:px-7 sm:py-7">
        <IssueAttentionSummaryCard attentionItems={attentionItems} variant="embedded" />
        ...
        <section className="space-y-3 border-t border-[--metis-outline-subtle] pt-6">
          ...
          {recentActivity.length > 0 ? (
            ...
            <p className="text-sm leading-6 text-[--metis-paper-muted]">No activity recorded yet.</p>
          )}
        </section>

        <section className="border-t border-[--metis-outline-subtle] pt-6">
          <IssueLifecyclePanel
            issueId={issueId}
