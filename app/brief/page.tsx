...
async function ensureLatestBriefVersion(issueId: string, mode: BriefMode) {
  ...
  return latest; // read-only: do not generate on page load
}
          ...
          {artifact ? (
            ...
            <article className="space-y-8 px-6 py-6 sm:px-7 sm:py-7">
              ...
              <div className="space-y-8">
                {artifact.executive.blocks.map((block, index) => (
                  <section key={block.label} className={index === 0 ? "space-y-3" : "space-y-3 border-t border-white/8 pt-8"}>
                    ...
                    <p className="max-w-4xl whitespace-pre-line text-lg leading-8 text-[--metis-paper]">{block.body}</p>
                  </section>
                ))}
              </div>
              ...
              <section className="space-y-4 border-t border-white/8 pt-8">
                ...
                <div className="space-y-4">
                  {artifact.executive.immediateActions.map((item) => (
            ...
            <article className="space-y-6 px-6 py-6 sm:px-7 sm:py-7">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <GenerateBriefButton issueId={activeIssue.id} mode={mode} />
              </div>
            </article>
          )}
        </SurfaceCard>
          ...
          <div className="divide-y divide-white/8">
            <div className="space-y-5 px-5 py-5">
              <div className="grid gap-3 rounded-[1.2rem] border border-white/8 bg-[rgba(0,0,0,0.16)] px-4 py-4 text-sm leading-6 text-[--metis-paper-muted]">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Badge className="border-0 bg-white/8 text-[--metis-paper-muted]">{activeIssue.status}</Badge>
                </div>
                <div className="flex items-center justify-between gap-3 border-t border-white/8 pt-3">
                  <span className="text-[0.62rem] uppercase tracking-[0.16em] text-[--metis-ink-soft]">Open gaps</span>
                  <Badge className="border-0 bg-[rgba(124,78,18,0.6)] text-amber-50">{activeIssue.openGapsCount}</Badge>
                </div>
              ...
              <div className="space-y-4 rounded-[1.2rem] border border-white/8 bg-[rgba(0,0,0,0.16)] px-4 py-4">
                ...
                <div className="space-y-3">
                  {blockers.length ? (
                    ...
                    <p className="text-sm text-[--metis-paper-muted]">No blockers detected in Sprint 1.</p>
                  )}
                </div>
                ...
                <div className="space-y-3 border-t border-white/8 pt-3">
                  ...
                  {evidenceItems.length ? (
                    ...
                    <p className="text-sm text-[--metis-paper-muted]">No evidence linked in Sprint 1.</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-3 px-5 py-5">
              ...
              <Button asChild variant="outline" className="w-full rounded-full border-white/10 bg-white/[0.03] text-[--metis-paper] hover:bg-white/[0.08]">
                <Link href="/sources" aria-disabled="true">
