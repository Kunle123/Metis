...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (isIssueArchived(issue)) {
    return NextResponse.json({ error: "Issue is already archived" }, { status: 409 });
  }
  ...
  await prisma.$transaction(async (tx) => {
    await tx.issue.update({
      where: { id: issue.id },
      data: { archivedAt: now, archivedById: ctx.user.id },
    });
    await writeIssueActivity(tx, {
      issueId: issue.id,
      kind: IssueActivityKinds.issue_archived,
      summary: "Issue archived",
      refType: "Issue",
      refId: issue.id,
