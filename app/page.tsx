...
import { DashboardAboutDisclosure } from "@/components/dashboard/DashboardAboutDisclosure";
import { DashboardMobileIssueCard } from "@/components/dashboard/DashboardMobileIssueCard";
import { DashboardMobileSummary } from "@/components/dashboard/DashboardMobileSummary";
import { DashboardMobileToolbar } from "@/components/dashboard/DashboardMobileToolbar";
      ...
      <div className="space-y-8 md:space-y-8">
        ...
        <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(248px,320px)]">
          <SurfaceCard className="min-w-0 overflow-hidden">
            {/* Mobile-first: compact mast + stats + ledger toggle */}
            <div className="border-b border-[--metis-outline-subtle] bg-[color-mix(in_oklab,var(--metis-surface-toolbar)_38%,transparent)] px-3 py-3 md:hidden">
              <DashboardMobileToolbar
                prepareOutputHref={firstIssue ? `/issues/${firstIssue.id}/export` : "/setup"}
                prepareDisabled={!firstIssue}
              />
              <DashboardMobileSummary
                ledger={ledger}
                ledgerIssueCount={issues.length}
                attentionCount={attentionCount}
                openQuestionSlots={workspacePulse.totalTrackedOpenQuestionSlots}
                savedExports={workspacePulse.totalStoredExportPackages}
              />
              <div className="mt-3 flex w-full flex-wrap items-center gap-2">
                <DashboardLedgerFilter ledger={ledger} className="min-w-[11rem] flex-1 md:min-w-0 md:flex-initial" />
                <span className="shrink-0 text-xs tabular-nums text-[--metis-text-tertiary]">({issues.length})</span>
              </div>
              <DashboardAboutDisclosure />
            </div>

            <div className="hidden border-b border-[--metis-outline-subtle] bg-[color-mix(in_oklab,var(--metis-surface-toolbar)_45%,transparent)] px-4 py-4 sm:px-6 md:block">
            ...
            <div className="space-y-3 px-3 py-3 md:space-y-4 md:px-5 md:py-5">
              ...
              {/* Mobile: secondary line when ledger has rows */}
              {issues.length > 0 ? (
                <p className="text-[0.65rem] leading-snug text-[--metis-text-tertiary] md:hidden">
                  Sorted by recent activity. Attention shows where to focus next.
                </p>
              ) : null}

              {issues.length === 0 ? (
                ledger === "archived" ? (
                  <div className="rounded-[1.45rem] border border-[--metis-outline-subtle] bg-[color-mix(in_oklab,var(--metis-surface-card)_72%,transparent)] px-4 py-8 text-center shadow-[inset_0_1px_0_color-mix(in_oklab,var(--metis-outline-strong)_10%,transparent)] md:px-6 md:py-10">
                    ...
                    <p className="mt-3 text-sm leading-relaxed text-[--metis-paper-muted]">
                      Archived issues appear here after you archive them from an issue record. Deleted issues are not shown. Switch to Active to return
                ...
                <>
                  <div className="flex flex-col gap-3 md:hidden">
                    {issues.map((issue) => (
                      <DashboardMobileIssueCard key={issue.id} issue={issue} />
                    ))}
                  </div>
