 ...
 * Editorial Situation Room. This shell should feel like a calm executive briefing desk,
 * with a dark navigation spine, restrained typography, document-like surfaces, and
 * visible evidence hierarchy. Favor precision, asymmetry, and editorial rhythm over
 * generic SaaS chrome.
 */
...
import { issueQueue, issueStats, navGroups, primaryNav, type ConfidenceLevel } from "@/lib/metis-data";
import { readinessTone, type ReadinessState } from "@/lib/metis-readiness";
...
const readinessClassMap: Record<ReadinessState, string> = {
  "Open gap": "border-[rgba(227,176,73,0.5)] bg-[rgba(131,82,17,0.72)] text-amber-50 ring-1 ring-[rgba(245,191,88,0.28)] shadow-[0_10px_24px_rgba(88,58,12,0.2),inset_0_1px_0_rgba(255,255,255,0.06)]",
  "Needs validation": "border-[rgba(220,112,136,0.52)] bg-[rgba(114,31,55,0.76)] text-rose-50 ring-1 ring-[rgba(241,137,160,0.28)] shadow-[0_10px_24px_rgba(70,17,32,0.24),inset_0_1px_0_rgba(255,255,255,0.06)]",
  "Source conflict": "border-[rgba(218,118,134,0.52)] bg-[rgba(90,33,47,0.78)] text-rose-50 ring-1 ring-[rgba(235,138,152,0.28)] shadow-[0_10px_24px_rgba(67,19,31,0.24),inset_0_1px_0_rgba(255,255,255,0.06)]",
  "Updated since last version": "border-[rgba(112,191,232,0.48)] bg-[rgba(19,86,118,0.72)] text-sky-50 ring-1 ring-[rgba(138,214,250,0.24)] shadow-[0_10px_24px_rgba(14,48,73,0.22),inset_0_1px_0_rgba(255,255,255,0.06)]",
  "Ready for review": "border-[rgba(148,169,194,0.38)] bg-[rgba(49,63,82,0.72)] text-slate-50 ring-1 ring-[rgba(203,213,225,0.18)] shadow-[0_10px_24px_rgba(23,30,40,0.22),inset_0_1px_0_rgba(255,255,255,0.05)]",
  "Ready to circulate": "border-[rgba(76,198,143,0.48)] bg-[rgba(18,91,60,0.76)] text-emerald-50 ring-1 ring-[rgba(108,226,168,0.24)] shadow-[0_10px_24px_rgba(15,54,38,0.22),inset_0_1px_0_rgba(255,255,255,0.06)]",
  Blocked: "border-[rgba(236,98,116,0.56)] bg-[rgba(116,23,31,0.8)] text-red-50 ring-1 ring-[rgba(248,127,143,0.28)] shadow-[0_10px_24px_rgba(73,15,20,0.26),inset_0_1px_0_rgba(255,255,255,0.06)]",
};
...
export function ReadinessPill({ state }: { state: ReadinessState }) {
  ...
  return (
    <span
      ...
      data-readiness-tone={readinessTone[state]}
    >
...
export function ScreenIntro({
  ...
  return (
      ...
      <div className="metis-surface metis-primary-surface relative overflow-hidden rounded-[1.8rem] border px-6 py-6 shadow-[0_24px_68px_rgba(0,0,0,0.24),inset_0_1px_0_rgba(255,255,255,0.04)] sm:px-7">

        <div className="absolute bottom-6 left-0 top-6 w-px bg-gradient-to-b from-transparent via-[--metis-brass] to-transparent opacity-70" />
        <div className="space-y-4 pl-3 sm:pl-5">
          <SectionEyebrow>{eyebrow}</SectionEyebrow>
          <div className="space-y-3">
            <h1 className="max-w-4xl font-[Cormorant_Garamond] text-4xl leading-[0.94] text-[--metis-paper] sm:text-5xl lg:text-[3.45rem]">
              {title}
            </h1>
            {description ? (
              <p className="max-w-3xl text-sm leading-7 text-[--metis-paper-muted] sm:text-base">
                {description}
              </p>
            ) : null}
          </div>
      ...
      {actions ? (
        <div className="metis-surface metis-support-surface rounded-[1.8rem] border p-5 shadow-[0_18px_48px_rgba(0,0,0,0.22),inset_0_1px_0_rgba(255,255,255,0.03)]">
          <div className="flex h-full flex-col justify-between gap-5">
            <div>
              <p className="text-[0.68rem] uppercase tracking-[0.24em] text-[--metis-ink-soft]">Actions</p>
            </div>
...
export function MetisShell({
  ...
  pageMeta,
  children,
  readinessState = "Ready for review",
  gapCount,
  ...
  return (
    <div className="min-h-screen bg-[--background] text-[--foreground]">
      ...
      <div className="relative grid min-h-screen lg:grid-cols-[286px_minmax(0,1fr)]">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 px-1">
                      <p className={cn("text-[0.62rem] uppercase tracking-[0.28em] text-[--metis-ink-soft]", groupIsActive && "text-[--metis-brass-soft]")}>{group}</p>
                    ...
                    <div className="space-y-2">
                      {items.map((item) => {
                        ...
                        return (
                          ...
                          >
                            <span className={cn("absolute inset-y-2 left-1 w-[6px] rounded-full bg-transparent transition duration-300", isActive && "bg-[--metis-brass-soft] shadow-[0_0_28px_rgba(224,183,111,0.5)]")} />
                            <span
                              className={cn(
                                "mt-1 h-2.5 w-2.5 rounded-full border border-white/10 bg-white/10 shadow-[0_0_0_4px_rgba(255,255,255,0.02)]",
                                isActive && "border-[--metis-brass-soft]/70 bg-[--metis-brass-soft] shadow-[0_0_0_5px_rgba(224,183,111,0.16)]",
                              )}
                            />
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center justify-between gap-4">
                                <div className="flex items-center gap-2">
                                  <span className={cn("text-sm font-medium text-[--metis-paper]", isActive && "text-white")}>{item.shortLabel}</span>
                                  {isActive ? (
                                    <span className="rounded-full border border-[--metis-brass]/20 bg-[--metis-brass]/12 px-2 py-0.5 text-[0.55rem] font-medium uppercase tracking-[0.22em] text-[--metis-brass-soft]">
                                      Active
                                    </span>
                                  ) : null}
                                </div>
              ...
              <div className="space-y-4 p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h3 className="text-base font-medium leading-6 text-[--metis-paper]">{activeIssue.title}</h3>
                  </div>
                  <Badge className="border-0 bg-rose-900/40 text-rose-100">{activeIssue.severity}</Badge>
                </div>
                <div className="grid grid-cols-2 gap-3 text-xs text-[--metis-paper-muted]">
                  <div className="rounded-2xl border border-white/8 bg-[rgba(0,0,0,0.16)] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                    ...
                    <div className="mt-2 text-xl text-[--metis-paper]">{activeIssue.openGaps}</div>
                  </div>
                  <div className="rounded-2xl border border-white/8 bg-[rgba(0,0,0,0.16)] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                    <div className="text-[0.68rem] uppercase tracking-[0.2em] text-[--metis-ink-soft]">Updated</div>
                    <div className="mt-2 text-sm text-[--metis-paper]">{activeIssue.updated}</div>
                  </div>
                </div>
              </div>
            </SurfaceCard>
        ...
        <main className="relative min-w-0 px-4 py-4 sm:px-6 sm:py-6 lg:px-8 lg:py-8">
          <div className="mx-auto flex min-h-[calc(100vh-2rem)] max-w-[1520px] flex-col overflow-hidden rounded-[2rem] border border-white/8 bg-[linear-gradient(180deg,rgba(18,23,24,0.94),rgba(10,14,15,0.985))] shadow-[0_32px_120px_rgba(0,0,0,0.52)] backdrop-blur-xl">
            ...
            {shouldShowOperationalSnapshot ? (
              ...
              <div className="border-b border-white/8 bg-[rgba(255,255,255,0.01)] px-5 py-2 sm:px-7 lg:px-8">
                <div className="flex flex-wrap items-center gap-1.5 text-[0.72rem] text-[--metis-paper-muted]">
                  <span className="max-w-[28rem] truncate text-[--metis-paper]">{activeIssue.title}</span>
                  <Badge className="border-0 bg-rose-900/40 text-rose-100">{activeIssue.severity}</Badge>
                  <ReadinessPill state={readinessState} />
                  <Badge className="border-0 bg-white/8 text-[--metis-paper-muted]">{effectiveGapCount} open gaps</Badge>
                  <span>{activeIssue.updated}</span>
                </div>
              </div>
            )}

            <div className="flex-1 bg-[linear-gradient(180deg,rgba(255,255,255,0.015),transparent_12%)] px-5 py-6 sm:px-7 lg:px-8">
              {children}
            </div>
