 ...
 * Editorial Situation Room. The dashboard should read like an active issue ledger,
 * not a marketing landing page. Use disciplined hierarchy, timeline cues, and
 * source-aware status language to create a leadership-ready operations surface.
 */
...
import { issueQueue, templateCards } from "@/lib/metis-data";
                ...
                <div className="flex flex-wrap gap-3">
                  <div className="relative min-w-[230px] flex-1 lg:w-[260px] lg:flex-none">
                    ...
                    <Input
                      value="portal outage"
                      readOnly
                      className="h-11 rounded-full border-white/10 bg-white/[0.04] pl-11 text-[--metis-paper]"
                    />
                  </div>
                ...
                >
                  <div className="space-y-4">
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge className="border-0 bg-white/8 text-[--metis-paper-muted]">{issue.type}</Badge>
                  ...
                  <div className="rounded-[1.35rem] border border-white/10 bg-[rgba(0,0,0,0.14)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                    <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1">
                      <div>
                        ...
                        <p className="mt-2 text-sm text-[--metis-paper]">{issue.owner}</p>
                      </div>
                      ...
                      <div className="border-t border-white/8 pt-4 sm:border-l sm:border-t-0 sm:pl-4 sm:pt-0 lg:border-l-0 lg:border-t lg:pl-0 lg:pt-4">
                        ...
                        <p className="mt-2 text-sm text-[--metis-paper]">{issue.updated}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="rounded-[1.35rem] border border-white/10 bg-[rgba(255,255,255,0.03)] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                    <div className="grid grid-cols-2 gap-3 border-b border-white/8 pb-4">
                      <div className="rounded-[1rem] border border-white/10 bg-[rgba(0,0,0,0.16)] p-3">
                        ...
                        <div className="mt-2 text-2xl text-[--metis-paper]">{issue.openGaps}</div>
                      </div>
                      <div className="rounded-[1rem] border border-white/10 bg-[rgba(0,0,0,0.16)] p-3">
                        <p className="text-[0.68rem] uppercase tracking-[0.24em] text-[--metis-ink-soft]">Sources</p>
                        <div className="mt-2 text-2xl text-[--metis-paper]">{issue.sources}</div>
                      </div>
                    </div>
                    <Link
                ...
                {templateCards.map((template) => (
                  ...
                  >
                    ...
                    <div className="mt-4 flex items-center justify-between gap-4 text-sm text-[--metis-paper-muted]">
                      ...
                      <Link
                        href="/setup"
                        className="inline-flex items-center gap-2 text-[--metis-brass-soft] transition hover:text-white"
                      >
                        Use template
                        <ArrowRight className="h-4 w-4" />
                      </Link>
            ...
            <SurfaceCard className="overflow-hidden">
              ...
              <div className="space-y-4 p-6">
                {recentUpdates.map((update, index) => (
                  <div key={update.time} className="grid grid-cols-[26px_minmax(0,1fr)] gap-4 rounded-[1.3rem] border border-white/10 bg-[rgba(255,255,255,0.025)] p-4">
